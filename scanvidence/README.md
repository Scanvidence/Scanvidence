# B0 — compact SegResNet-style CNN (Scanvidence)
...
(~5 s per test on a modern CPU).

## Quick start (Windows)

```bat
:: 1. Install environment
conda create -n scanvidence python=3.11 -y
...
- 1 251 unique 2023 cases, 1 255 rows (4 blank).
- Labels: 0=background, 1=necrotic core, 2=edema, **3=enhancing tumor**.
- The data loader enforces the 2023 convention and refuses label 4 unless
  `--remap-legacy-four` is passed.

---
*This README summarises the B0 reference architecture and the training pipeline.
For full work‑plan details, see the Scanvidence `DEVELOPMENT.md` at the root.*